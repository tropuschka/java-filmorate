...
public class UserService {
    ...
    public Collection<Long> addUserFriend(Long userId, Long friendId) {
        ...
        if (user.getFriends() != null && friend.getFriends() != null) {
            if (user.getFriends().containsKey(friend.getId()) // сделала &&, чтобы в случае, если дружба односторонняя,
                    && friend.getFriends().containsKey(user.getId())) { // она обновлялась до двусторонней
    ...
    public Collection<Long> deleteUserFriend(Long userId, Long friendId) {
        ...
        if (user.getFriends().containsKey(friend.getId())) user.deleteFriend(friend);
        if (friend.getFriends().containsKey(user.getId())) friend.deleteFriend(user);
